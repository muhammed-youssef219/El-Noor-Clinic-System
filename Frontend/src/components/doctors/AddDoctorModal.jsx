...
const EMPTY_DAY = { checked: false, from: "09:00", to: "17:00" };

export default function AddDoctorModal({ onClose, onCreated, actor }) {
  const [form, setForm] = useState({
    name: "", specialtyKey: SPECIALTIES[0].key, email: "", license: "", exp: "", price: "", followUp: "", bio: "", photo: null,
  });
  const [days, setDays] = useState(() => Object.fromEntries(WEEKDAY_AR.map(d => [d, { ...EMPTY_DAY }])));
  ...
  function updateDay(day, key, value) {
    setDays(prev => ({ ...prev, [day]: { ...prev[day], [key]: value } }));
  }
  ...
  return (
    <Modal title="إضافة دكتور جديد" onClose={onClose} width={560}>
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <PhotoUploadField name={form.name || "دكتور"} photo={form.photo} onChange={photo => update("photo", photo)} />
        <div>
          <label>الاسم الكامل *</label>
          <input value={form.name} onChange={e => update("name", e.target.value)} placeholder="د. ..." />
        </div>
        ...
        <div>
          ...
          <input className="mono" type="email" value={form.email} onChange={e => update("email", e.target.value)} placeholder="name@clinic.eg" />
        </div>
        ...
        <button className="btn btn-primary" type="submit" disabled={!canSave} style={{ opacity: canSave ? 1 : 0.5, alignSelf: "flex-start" }}>
          <Check size={15} /> {saving ? "جاري الإضافة…" : "إضافة الدكتور"}
        </button>
      </form>
    </Modal>
  );
